...
E.1.u.m.lam <-function(lambda, S, type = c("u", "U"), spouse = c("h", "w")){
  ...
  if (!missing(spouse)){
    if (type == "u"){
      if (spouse == "h"){
        U.2.m.vec <- mapply(function(h) u(h), c2.m.array.lam(lambda, S)[,1,,])
      }
      if (spouse == "w"){
        U.2.m.vec <-  mapply(function(w) u(w), c2.m.array.lam(lambda, S)[,2,,])
      }
