...
period.1.d <- function(i, r){
  ...
  eval_f_1_d <- function(c, i, r){
    return(list("objective" = - U(c[1], c[2])- beta * E.1.U.d(c[3], type = "U")[i,r], 
                "gradient" = c(-wt.h * u.grad(c[1]), -wt.w * u.grad(c[2]), (-beta) * E.1.u.d.der(c[3], type = "U")[i,r] )))
  }
  ...
  lb <- c(0, 0, -Inf)
  res <- tryCatch(nloptr(x0 = c0, eval_f = eval_f_1_d, lb = lb, ub = ub, 
                eval_g_eq = eval_g_eq_1, opts = opts, i = i, r = r), error = function(e) NULL)
  return(res$solution)
}