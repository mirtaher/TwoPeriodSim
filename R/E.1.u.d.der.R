...
E.1.u.d.der <-function(S, type = c("u", "U"), spouse = c("h", "w")){
  ...
  if (!missing(spouse)){
    if (type == "u"){
      if (spouse == "h"){
        U.2.m.vec <- mapply(function(h) u.grad(h) * delta * 1/beta , c2.d.array(S)[,1,,])
      }
      if (spouse == "w"){
        U.2.m.vec <- mapply(function(w) u.grad(w) * (1-delta) * 1/beta, c2.d.array(S)[,2,,])
      }
    }
    
    if (type == "U"){
      U.2.m.vec <- mapply(function(h,w) wt.h * u.grad(h) * delta * 1/beta + wt.w * u.grad(w) * (1-delta) * 1/beta, 
                          c2.d.array(S)[,1,,], c2.d.array(S)[,2,,])
    }
  } else {
    U.2.m.vec <- mapply(function(h,w) wt.h * u.grad(h) * delta * 1/beta + wt.w * u.grad(w) * (1-delta) * 1/beta, 
                        c2.d.array(S)[,1,,], c2.d.array(S)[,2,,])
